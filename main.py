...
def create_output_directories():
    ...
    try:
        ...
        # 创建可视化结果目录
        for method in ["Grad-CAM", "Score-CAM", "Layer-CAM"]:  # 移除LIME和SHAP
            for model in ["ResNet50", "MobileNet"]:
                os.makedirs(os.path.join("output", "visualizations", method, model, "train"), exist_ok=True)
                os.makedirs(os.path.join("output", "visualizations", method, model, "test"), exist_ok=True)
